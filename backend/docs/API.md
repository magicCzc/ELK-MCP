  ...
  - 入参（见 zod �?Pydantic）：
    ...
    - `filters`: `{ level?: string[], service?: string[], keyword?: string, fuzzy_type?: string, fuzzy_options?: object }`
      ...
      - `fuzzy_type`: 模糊匹配类型，可选值：
        - `"contains"` (默认): 包含匹配，使用最佳字段匹�?
        - `"prefix"`: 前缀匹配，匹配以关键词开头的日志
        - `"fuzzy"`: 模糊匹配，容忍拼写错误（�?"erorr" 能匹�?"error"�?
        - `"wildcard"`: 通配符匹配，支持 `*`（任意字符）�?`?`（单个字符）
        - `"regexp"`: 正则表达式匹配，功能最强但性能最�?
    ...
    - `query_string?: string` - Lucene 查询语法支持（可选，�?filters.keyword 互斥�?
      - 优先级：query_string > filters.keyword（如果提�?query_string，keyword 被忽略）
    ...
    - 动态索引选择与集群精准路由：
      - `index_keyword?: string` 用于按项�?索引名关键字路由到特�?ES 集群（如 "KF1"）。系统会根据 `.env` 中的 `ES_PROJECT_MAP` 配置进行毫秒级精准路由，大幅减少跨集群扫描�?
...
**⚠️ 安全提示**：必须提�?`index_keyword` �?`override_indexes` 明确指定索引，禁止查询所有索�?
...
**⚠️ 注意**：前导通配符（�?`*exception`）已被禁止，会自动移�?
...
**⚠️ 警告**：正则表达式查询性能最差，建议仅在必要时使用，并严格限制时间范�?
...
**⚠️ 安全提示**：所有示例都必须包含 `index_keyword` �?`override_indexes` 明确指定索引
...
1. **必须指定索引**：每次查询必须提�?`index_keyword` �?`override_indexes`，禁止查询所有索�?
2. **索引数量限制**：单次查询最多支�?10 个索引，超出部分会被自动截断
3. **禁止前导通配�?*：`*keyword` 形式的查询会被自动移除前导通配�?
4. **建议时间范围**：建议将时间范围限制�?24 小时内，减少扫描数据�?
5. **并发限制**：多集群并发查询最�?3 个线程，防止压垮 ES

### 错误码说�?

- `3002` (INVALID_PARAM)：未指定索引或索引无�?
- `2001` (ES_CONNECTION)：ES 连接失败或查询超�?
...
- `POST /api/logs/paginate/get`
  ...
  - 出参�?
    ...
    ```
  - 会话过期：默�?小时过期，过期后需重新初始�?
  - 容错：页码超出范围时返回错误�?

### 使用场景

1. **大量数据查询**：当查询结果非常大时，使用分页会话可以避免一次性加载过多数�?
2. **稳定的分页体�?*：会话有效期内，查询条件保持不变，分页结果更稳定
3. **降低ES负载**：通过会话缓存查询条件，减少重复构建ES DSL的开销
4. **简化前端逻辑**：前端只需维护会话ID和当前页码，无需重复传递复杂查询条�?
...
- 实时扫描：通过后台线程�?`interval_seconds` 周期拉取 `/_cat/indices` 并缓存�?
- 匹配机制：支持正则与关键字匹配；无命中时自动进行模糊匹配（大小写不敏感）�?
- 并行与降级：多集群并发查询；当索引过多或查询超时，自动缩减索引列表并重试�?
- 监控与指标：
  - `mcp_index_refresh_total` 刷新次数
  - `mcp_index_count` 当前缓存索引�?
  - `mcp_index_match_ratio` 索引匹配成功率（匹配�?缓存数）
...
- RBAC 拒绝：`error.rbac.denied`（HTTP 200，`code` �?0�?
- ES 连接异常：`error.es.connection`
- 输入不合法：`error.input.bad` �?HTTP 422（Pydantic 校验失败�?
- 索引配置错误：`error.indices.bad_config`

## 认证与权�?

- 通过 `Authorization: Bearer <token>` �?`X-Tenant-Id` 控制访问�?
- RBAC 基于配置文件与请求上下文进行资源与动作校验�?

## ES 6.5.4 适配说明

- 支持 `doc_type` 与旧�?endpoint 参数；查�?DSL 在转换层进行差异处理�?
